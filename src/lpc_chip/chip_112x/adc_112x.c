...
void Chip_ADC_Init(LPC_ADC_T *pADC, uint32_t flags)
{
	...
	Chip_SYSCON_PowerUp(SYSCON_POWERDOWN_ADC_PD);
	Chip_Clock_EnablePeriphClock(SYSCON_CLOCK_ADC);

	/* FIXME - LPC1125 UM describes ADC reset, but no ADC reset bit in SYSCON.
...
void Chip_ADC_DeInit(LPC_ADC_T *pADC)
{
	...
	Chip_SYSCON_PowerDown(SYSCON_POWERDOWN_ADC_PD);
}
