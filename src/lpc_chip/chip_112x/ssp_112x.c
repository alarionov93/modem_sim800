...
{
	CHIP_SYSCON_CLOCK_T clkSSP;

	if (pSSP == LPC_SSP0) {
		clkSSP = SYSCON_CLOCK_SSP0;
	}
	else {
		clkSSP = SYSCON_CLOCK_SSP1;
	}
...
{
	CHIP_SYSCON_PERIPH_RESET_T resetSSP;
...
void Chip_SSP_Init(LPC_SSP_T *pSSP)
{
	...
	Chip_SYSCON_PeriphReset(Chip_SSP_GetResetIndex(pSSP));
